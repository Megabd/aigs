...
def crossover(x1, x2):  # We interlop between the two parents
    a = np.random.rand()
    return x1 * a + x2 * (1 - a)
...
def main(cfg):
    pop = np.random.uniform(-50.0, 50.0, size=(cfg.population, 2))
...
def fitness(stats):
    ...
    s_distfloor = low_good_cap(stats["dist-floor"], 10.0)
    s_disjtube  = low_good_cap(stats["disjoint-tubes"], 10.0)
    s_noise     = low_good_cap(stats["noise"], 100.0)
    s_empty     = high_good_cap(stats["empty"], 500.0)

    secondary = (s_distfloor + s_disjtube + s_noise + s_empty) * 5

    primary = -stats["dist-win"]
...
def mutate_map(level_map, num_mutations=1, num_tiles=None):
    """
    Return a slightly modified copy of the map.
    
    - level_map: numpy array (e.g., shape (14, 114))
    - num_mutations: how many tiles to change
    - num_tiles: total number of tile types (if None, inferred from map max+1)
    """
...
def MAP_Elite(cfg, env):
    ...
    for i in range(cfg.budget):
        if (i < G):
            ...
            x = mutate_map(rec["solution"], num_mutations=100, num_tiles=env.get_num_tiles())
        stats = env._prob.get_stats(get_string_map(x, env._prob.get_tile_types()))
        b = stats["jumps"]
        p = fitness(stats)
        key = get_key([stats["jumps"] / maxJumps], resolution)
        if key not in archive or archive[key]["fitness"] < p:
            archive[key] = {"fitness": p, "behavior": b, "solution": x}
    return archive
...
def main(cfg):
    ...
    target_key = get_key([10/100.0], 10)
...
def init_pcgym(cfg) -> Tuple[PcgrlEnv, np.ndarray]:
    ...
    env.reset()
    pop = np.random.randint(0, env.get_num_tiles(), (cfg.budget, *env._rep._map.shape))  # type: ignore
    return env, pop
